...
CREATE POLICY "Tenant admins can manage their students" ON public.students
  FOR ALL USING (get_current_user_role() = 'tenant_admin' AND tenant_id = get_current_user_tenant());

CREATE POLICY "Teachers can view tenant students" ON public.students
  FOR SELECT USING (get_current_user_role() = ANY(ARRAY['teacher', 'staff']) AND tenant_id = get_current_user_tenant());

CREATE POLICY "Students can view their own profile" ON public.students
  FOR SELECT USING (get_current_user_role() = 'student' AND user_id = auth.uid());

CREATE POLICY "Students can update their own profile" ON public.students
  FOR UPDATE USING (get_current_user_role() = 'student' AND user_id = auth.uid());
...
CREATE POLICY "Teachers can manage their subject records" ON public.student_academic_records
  FOR ALL USING (get_current_user_role() = 'teacher' AND teacher_id = auth.uid() AND tenant_id = get_current_user_tenant());
...
CREATE POLICY "Teachers can manage tenant attendance" ON public.student_attendance
  FOR ALL USING (get_current_user_role() = ANY(ARRAY['teacher', 'staff']) AND tenant_id = get_current_user_tenant());
